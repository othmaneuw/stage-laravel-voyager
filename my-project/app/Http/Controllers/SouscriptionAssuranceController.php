...
class SouscriptionAssuranceController extends VoyagerBaseController{
    public function index(Request $request)
    {
        ...
        $user = Auth::user();
        if ($user["role_id"] !== 1 && $user["role_id"] !== 3 && $user["role_id"] !== 4 && $user["role_id"] !== 6) {
    ...
    public function store(Request $request)
    {
        ...
        && Auth::user()['role_id'] !== Role::where('name', 'Gestionnaire AOS')->get()->first()->id
         && Auth::user()['role_id'] !== Role::where('name', 'Gestionnaire Assurance')->get()->first()->id
        ){
    ...
    public function update(Request $request, $id)
    {
        ...
        $souscription_assurance_statut = DB::table('souscriptionassurances')->where('id',$id)->get()->first()->statut;
        $reservation_date = DB::table('souscriptionassurances')->where('id',$id)->get()->first()->created_at;
        $updated_value = $request->request->get('statut');
        $user = DB::table('users')->where('id',$request->request->get('user'))->get()->first(); 
        $assurance = DB::table('assurances')->where('id',$request->request->get('assurance'))->first();
        $assurance_name = $assurance->nom;
        $user_email = $user->email;
        $user_name = $user->name;
        if($souscription_assurance_statut !== $updated_value){
