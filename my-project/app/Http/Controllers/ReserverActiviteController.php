...
class ReserverActiviteController extends VoyagerBaseController{
    ...
    public function update(Request $request, $id)
    {
        ...
        $reservation_statut = DB::table('reservation_activites')->where('id',$id)->get()->first()->statut;
        $reservation_date = DB::table('reservation_activites')->where('id',$id)->get()->first()->created_at;
        $updated_value = $request->request->get('statut');
        $user = DB::table('users')->where('id',$request->request->get('user'))->get()->first(); 
        $activite = DB::table('activites')->where('id',$request->request->get('activite'))->first();
        $activite_name = $activite->nom;
        $user_email = $user->email;
        $user_name = $user->name;
        if($reservation_statut !== $updated_value){
            if($updated_value == "refused"){
                Mail::to($user_email)->send(new Rejection($user_name,$activite_name,$reservation_date));
            }else if($updated_value == "Validated"){
                Mail::to($user_email)->send(new Validation($user_name,$activite_name,$reservation_date));
            }
        }
